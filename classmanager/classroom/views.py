...
def excel_upload(request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                if excel_file.name.endswith(('.xls', '.xlsx')):
                    ...
                    messages.error(request, 'Invalid file format. Use .xls, .xlsx, or .csv.')
                    return render(request, 'classroom/excel_upload.html', {'form': form})
    ...
    # For students or non-POST requests, display the latest data (example: stored in session)
    if not request.user.is_teacher:
        excel_data = request.session.get('excel_data', None)

    return render(request, 'classroom/excel_upload.html', {
        'form': form,
